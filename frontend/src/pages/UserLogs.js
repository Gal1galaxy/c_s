  ...
  const fetchLogs = async (page = 1, pageSize = 20) => {
    ...
    try {
      ...
      if (filters.dateRange) {
        const start_date = filters.dateRange[0].startOf('day').utc().format('YYYY-MM-DD HH:mm:ss');
        const end_date = filters.dateRange[1].endOf('day').utc().format('YYYY-MM-DD HH:mm:ss');
